...
import { ThumbsUp, Edit3, Trash2, ChevronDown, ChevronUp, AlertTriangle } from 'lucide-react'
...
export function ReviewCard({ review, onAction, style }: ReviewCardProps) {
  ...
  return (
    <div
      className="card card-hover p-5 stagger-child"
      style={style}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {review.sentiment && (
            <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${sentimentColors[review.sentiment as keyof typeof sentimentColors] ?? 'text-secondary bg-surface border-border'}`}>
              {review.sentiment}
            </span>
          )}
          {(review.flags as string[]).map(flag => (
            <span key={flag} className={`text-xs px-2 py-0.5 rounded-full border font-medium flex items-center gap-1 ${flagColors[flag as keyof typeof flagColors] ?? 'text-warning bg-warning/10 border-warning/20'}`}>
        ...
        <div className="mb-4">
          ...
          {isLong && (
            <button
              ...
              className="flex items-center gap-1 text-xs text-muted hover:text-secondary mt-1 transition-colors"
            >
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-accent font-medium">AI draft reply</span>
          </div>
          {!editing && (
            <button
        ...
        {editing ? (
          <div>
            ...
            <div className="flex gap-2 mt-2">
              ...
              >
                {loading === 'edit_and_post' ? <Spinner size={13} /> : null}
                Post edited reply
              </button>
              <button
          ...
          <div
            className="bg-accent/5 border border-accent/20 rounded-[8px] p-3 cursor-text"
            onClick={() => setEditing(true)}
          >
      ...
      {!editing && (
        <div className="flex gap-2 flex-wrap">
          ...
          >
            {loading === 'approve' ? <Spinner size={14} /> : <ThumbsUp size={14} />}
            Approve
          </button>
          <button
            onClick={() => setEditing(true)}
          ...
          <button
            ...
            disabled={!!loading}
            className="btn btn-ghost text-sm px-4 py-2 text-muted hover:text-secondary"
          >
